...
$fallao= array();
    ...
    $sql1="SELECT cliente FROM ordenservicio WHERE id = '$orden'";
    $res1 = mysqli_query($con,$sql1);
    while($row1=mysqli_fetch_array($res1)){
      $cliente[]=$row1['cliente'];
    };
    $sql1="SELECT feccreacion FROM ordenservicio WHERE id = '$orden'";
    $res1 = mysqli_query($con,$sql1);
    while($row1=mysqli_fetch_array($res1)){
      $feccreacion[]=$row1['feccreacion'];
    };
$sql1="SELECT fecha FROM ordenservicio WHERE id = '$orden'";
    $res1 = mysqli_query($con,$sql1);
    while($row1=mysqli_fetch_array($res1)){
      $fechao[]=$row1['fecha'];
    };

$sql1="SELECT servicio FROM ordenservicio WHERE id = '$orden'";
    $res1 = mysqli_query($con,$sql1);
    while($row1=mysqli_fetch_array($res1)){
    ...
    $sql1= "SELECT clave FROM accesoservicio WHERE orden = '$orden'";
    $res1 = mysqli_query($con,$sql1);
    while($row1=mysqli_fetch_array($res1)){
      $clave[]=$row1['clave'];
    };  
...
//Si no hay correo pero si hay celular
 ...
 echo "<script type=\"text/javascript\">window.location = 'https://wa.me/52$celular/?text=Para visualizar su órden de servicio deberá ir al siguiente link: http://odin.nextcam.com.mx/servicios/porden3.php y una vez en el sitio introducir su clave: $clave[0]';</script>";
//FIN DE ENVIO POR CELULAR
}elseif($tipodeenvio==1){
...
';
  ...
  if(mail($to, $subject, $message, $headers)){
   ...
   echo "<script type=\"text/javascript\">alert(\"No puedes acceder aqui\");window.location = 'orealizadas.php';</script>";
}
