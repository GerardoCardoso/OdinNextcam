...
<head>
  ...
  <title>Ordenes Realizadas | NextAdmin</title>
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Órdenes Realizadas</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
...
for ($i=0; $i < $firmas ; $i++) { 
  $sql1="SELECT id FROM ordenservicio";
    $res1 = mysqli_query($con,$sql1);
    while($row1=mysqli_fetch_array($res1)){
      $id[]=$row1['id'];
    };
$sql1="SELECT fecha FROM ordenservicio";
    $res1 = mysqli_query($con,$sql1);
    while($row1=mysqli_fetch_array($res1)){
      $fechao[]=$row1['fecha'];
    };
    $sql2="SELECT cliente FROM ordenservicio";
    $res2 = mysqli_query($con,$sql2);
    while($row2=mysqli_fetch_array($res2)){
      $cliente[]=$row2['cliente'];
    };  
  $sql12="SELECT tecnico FROM ordenservicio";
    $res12 = mysqli_query($con,$sql12);
    while($row12=mysqli_fetch_array($res12)){
      $tecnico[]=$row12['tecnico'];
    };  
    $sql3="SELECT servicio FROM ordenservicio";
    $res3 = mysqli_query($con,$sql3);
    while($row3=mysqli_fetch_array($res3)){
      $servicio[]=$row3['servicio'];
    };
  $sql3="SELECT costo FROM ordenservicio";
    $res3 = mysqli_query($con,$sql3);
    while($row3=mysqli_fetch_array($res3)){
      $costo[]=$row3['costo'];
    };
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <?php  for ($i=0; $i <$firmas; $i++) { 
                          ...
                          echo '<td>'.money_format('%(#10n', $costo[$i]).'</td>';
                           echo '<td><form action"porden2.php" method="post" target="_parent">
         <input type="hidden" name="codigo" value="'.$id[$i].'" />
         <button id="b1" class="btn btn-link" title="Visualizar" type="submit" formaction="porden2.php" style="color:black;" >Ver Órden</button></form></td>';
                          echo '</tr>';
                          } ?>
