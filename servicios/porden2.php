...
if($conteo>15){
  $dir = substr($direccion[0], 0,15);
  
}else{
  
  $dir=$direccion[0];
}
$conteo2=strlen($colonia[0]);
if($conteo2>12){
  $col = substr($colonia[0], 0,15);
...
$pdf->Cell(0,0,utf8_decode('DIRECCION: '.strtoupper ($dir).' # '.$numero[0].', '.strtoupper ($col).', '.strtoupper ($nmunicipio[0]).''),0,1,'R');
$pdf->Ln(5);
...
$pdf->Cell(0,0,utf8_decode('CARACTERISTICAS: '.substr(strtoupper ( $caracteristicas[0]), 0, 15).''),0,1,'C');
...
$pdf->Ln(3);
$pdf->SetFont('Arial','',8);
$pdf->MultiCell(0,5,utf8_decode(''.substr(strtoupper ( $observaciones[0]),0, 30).''));
